WITH
    param
    as
    (SELECT TO_DATE($P{Start_Date}) PSTARTDATE, TO_DATE($P{End_Date}) PENDDATE,  $P{irb}  PIRBC,  $P{MGMT_GROUP} PMGMT_GROUP FROM DUAL)
...
FROM uacc_oncore_rw_utils.protocols p 
    ...
    where 
              ...
              and REVIEW_REASON IN ('Continuing Review','Initial Review')
