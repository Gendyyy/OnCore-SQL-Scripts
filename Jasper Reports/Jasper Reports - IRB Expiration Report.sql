...
( $P{End_Date} ) PENDDATE,  $P{irb}  PIRBC,  $P{MGMT_GROUP} PMGMT_GROUP FROM DUAL)
, irbDATES AS
(select protocol_id, PROTOCOL_NO, MAX(ACTION_DATE) maxeffectivedate, MAX(EXPIRATION_DATE) maxexpireddate
from uacc_oncore_prod.rv_pcl_irb_review
where REVIEW_REASON IN ('Continuing Review','Initial Review') and review_expires = 'Y'
group by protocol_no, protocol_id
              )
SELECT p.PI_Name piname, p.protocol_no, p.study_target_accrual, p.title, p.total_accrual totalonstudy,
    i.maxeffectivedate, i.maxexpireddate,
    g.sponsor_name, g.sponsor_protocol_no,
...
FROM uacc_oncore_rw_utils.protocols p 
    ...
    FROM uacc_oncore_rw_utils.sv_pcl_staff_ro
    WHERE staff_role = 'Primary IRB Coordinator' AND ACTIVE_FLAG = 'Y' and protocol_subject_id is null ) c ON p.protocol_id = c.protocol_id
    LEFT JOIN IRBDATES I ON I.PROTOCOL_ID = P.PROTOCOL_ID
WHERE CURRENT_status NOT IN ( 'ABANDONED', 'TERMINATED', 'IRB STUDY CLOSURE', 'NEW','SRC APPROVAL' )
    AND maxexpireddate BETWEEN PSTARTDATE AND PENDDATE
    and review_expires = 'Y'
    AND ((irbcoord_name LIKE '%' || PIRBC || '%' and (stop_date > sysdate or stop_date is null)) OR PIRBC IS NULL)
    AND (ALL_DMG LIKE  '%' || PMGMT_GROUP || '%' OR PMGMT_GROUP IS NULL )
    and g.principal_sponsor = 'Y'
order by PI_NAME, PROTOCOL_NO