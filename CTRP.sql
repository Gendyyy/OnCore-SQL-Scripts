WITH prot AS (SELECT t.*,
                     ROWNUM                                                                AS subject_no,
                     ...
                     (SELECT institution_name
                      FROM uacc_oncore_prod.sv_pcl_institution
                      WHERE protocol_id = t.protocol_id)                                   AS INSTITUTION
              FROM (SELECT *
                    FROM uacc_oncore_prod.smrs_pcl_accrual_summary t
                    WHERE t.protocol_id =
                          (SELECT protocol_id FROM protocols WHERE protocol_no = 'STUDY00001225')) t, TABLE (
                  CAST(MULTISET(SELECT ROWNUM r FROM DUAL CONNECT BY LEVEL <= t.accrual) AS sys.ODCINUMBERLIST)
                  ))
-- Retrieve 'COLLECTIONS' data
SELECT '"COLLECTIONS",' || '"' || TRIM(nci_CTRP_id) || '"' || ',,,,,,,,,' A
FROM UACC_ONCORE_RW_UTILS.mypv4
WHERE PROTOCOL_NO = 'STUDY00001225'
...
SELECT '"PATIENTS",' ||
       ...
       '"' || (SELECT MAX(org_id_type_identifier)
               FROM uacc_oncore_prod.rv_organization_identifier
               WHERE organization_name = P.INSTITUTION
                 AND organization_id_type_name = 'NCI PO ID') || '"' || ',,,,,,,,,,' ||
       '"199.1"' || ',,' A
FROM prot p
...
WHERE p.PROTOCOL_NO = 'STUDY00001225'
...
SELECT '"PATIENT_RACES",' ||
       '"' || TRIM(p.NCI_CTRP_ID) || '",' ||
       '"' || p.SUBJECT_NO || '",' ||
       '"' || case when r.DESCRIPTION is null then 'Unknown' else r.DESCRIPTION end || '"' A
FROM prot p
         LEFT JOIN UACC_ONCORE_PROD.SV_RACE R ON P.RACE = R.CODE_ID
WHERE p.PROTOCOL_NO = 'STUDY00001225'
-- ORDER BY p.subject_no;